    ...
    >
      ...
      >
        <button
          type="button"
          onClick={onClose}
          className="absolute right-3 top-3 text-2xl leading-none text-slate-400 hover:text-slate-700"
          aria-label="Закрити"
        >
          ×
        </button>
        ...
        <h2 className="pr-6 text-center text-lg font-semibold text-slate-900">
          Видалити це помешкання?
        </h2>

        <p className="mx-auto mt-2 max-w-[300px] text-center text-xs leading-4 text-slate-500">
          Помешкання буде видалено тільки з цього списку.
        </p>

        <p className="mt-2 line-clamp-2 text-center text-sm font-medium text-slate-700">
          «{housingTitle}»
        </p>
