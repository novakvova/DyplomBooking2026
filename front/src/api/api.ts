...
export const paymentsApi = {
  payHousingBooking: async (dto: {
    housingBookingId: number;
    method: PaymentMethod;
    cardLastFour?: string;
  }): Promise<Payment> => {
    const { data } = await apiClient.post('/payments/pay-housing-booking', dto);
    return data;
  },
  ...
  refund: async (paymentId: number): Promise<Payment> => {
    const { data } = await apiClient.post('/payments/refund', { paymentId });
    return data;
  },
};
...
export const PaymentStatusColor: Record<PaymentStatus, string> = {
  ...
  [PaymentStatus.Refunded]: 'text-slate-600 bg-slate-50',
};
...
export const wishlistApi = {
  ...
  add: async (housingId: number): Promise<void> => {
    await apiClient.post(`/wishlist/${housingId}`);
  },
